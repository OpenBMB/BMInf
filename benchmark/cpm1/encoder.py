...
def test(model : bminf.models.CPM1, input_tensor, mask):
    model._model.encode(
        model._ctx,
        input_tensor,
        mask
...
def main():
    ...
    print("Loading model")
    cpm1 = bminf.models.CPM1(device_idx=args.device)

    ctx = cpm1._ctx
    encoder_input = ctx.allocate((args.batch_size, cpm1._config.DIM_MODEL, args.input_length), np.half)
    encoder_mask = np.ones((args.batch_size, args.input_length), np.bool8)
    test(cpm1, encoder_input, encoder_mask)
